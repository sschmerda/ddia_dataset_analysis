...
def return_and_plot_evaluation_score_range(interactions: pd.DataFrame,
    ...
    -------
    dict
        A dictionary containing score ranges for each evaluation level (learning_activity, group and course)
    """
    ...
    for group, score, group_name in zip([learning_activity_field, group_field, 'course'], 
                                        [evaluation_learning_activity_score_field, evaluation_group_score_field, evaluation_course_score_field], 
                                        [LEARNING_ACTIVITY_FIELD_NAME_STR, GROUP_FIELD_NAME_STR, COURSE_FIELD_NAME_STR]):

        eval_score_ranges_dict[group_name] = {'eval_score_ranges': None,
                                              'range_count_frequency': None}
        if group and score:
            if (group == learning_activity_field) or (group == group_field):
                eval_score_ranges = interactions.groupby(group)[score].agg(func=[min, max, np.mean, np.median])
                eval_score_ranges = eval_score_ranges.rename(columns={'min': 'score_minimum', 'max': 'score_maximum', 'mean': 'score_mean', 'median': 'score_median'})
                eval_score_ranges = eval_score_ranges.reset_index()
                eval_score_ranges_long = pd.melt(eval_score_ranges, 
                                                 id_vars=[group], 
                                                 value_vars=['score_minimum', 'score_maximum'], 
                                                 var_name='boundary_type', 
                                                 value_name='boundary_value').sort_values(by=group)

            else:
                eval_score_ranges = interactions[score].agg(func=[min, max, np.mean, np.median])
                eval_score_ranges = pd.DataFrame(eval_score_ranges).transpose().reset_index()
                eval_score_ranges = eval_score_ranges.rename(columns={'index': group, 'min': 'score_minimum', 'max': 'score_maximum', 'mean': 'score_mean', 'median': 'score_median'})
                eval_score_ranges_long = pd.melt(eval_score_ranges, 
                                        id_vars=[group], 
                                        value_vars=['score_minimum', 'score_maximum'], 
                                        var_name='boundary_type', 
            ...
            range_series = pd.Series(eval_score_ranges[['score_minimum', 'score_maximum']].itertuples(index=False, name=None))
            range_count_df = range_series.value_counts().reset_index().rename(columns={'index': 'range', 0: 'count'})
            range_frequency_df = range_series.value_counts(normalize=True).reset_index().rename(columns={'index': 'range', 0: 'frequency'})
            range_counts_frequency_df = range_count_df.merge(range_frequency_df, how='left', on='range')
            ...
            eval_score_ranges_dict[group_name] = {'eval_score_ranges': eval_score_ranges,
                                                  'range_count_frequency': range_counts_frequency_df}
            
            are_equal_all_score_minima = eval_score_ranges['score_minimum'].nunique()==1
            are_equal_all_score_maxima = eval_score_ranges['score_maximum'].nunique()==1
            ...
            print(f'Minima of {group_name} Ranges are all equal: {are_equal_all_score_minima}')
            print(f'Maxima of {group_name} Ranges are all equal: {are_equal_all_score_maxima}')
            print(f'Minimum {group_name} Score: {score_minimum}')
            print(f'Maximum {group_name} Score: {score_maximum}')

            # scatter
            g = sns.JointGrid(data=eval_score_ranges, 
                              x='score_minimum', 
                              y='score_maximum')
            ...
            g.plot_marginals(sns.rugplot, 
                             height=1, 
                             alpha=0.6)
            g.set_axis_labels(xlabel=f'{group_name} Minimum Score', 
                              ylabel=f'{group_name} Maximum Score')
            plt.show()

            # barplot with min and max scores
            groups = sorted(list(np.unique(eval_score_ranges_long[group])), key=lambda x: int(x))
            mins = [eval_score_ranges_long[eval_score_ranges_long[group] == g]['boundary_value'].min() for g in groups]
            lengths = [eval_score_ranges_long[eval_score_ranges_long[group] == g]['boundary_value'].max() - g_mins for g, g_mins in zip(groups, mins)]

            mean_marker = {"marker":"o",
                            "markerfacecolor":"green", 
                            "markeredgecolor":"black",
                            "markersize":10}

            median_marker = {'linewidth': 5,
                            'linestyle': '-',
                            'color': 'blue'}

            g = sns.barplot(x=lengths, 
                            y=groups, 
                            left=mins, 
                            palette='turbo', 
                            orient='h')
            g = sns.stripplot(x='boundary_value', 
                              y=group, 
                              order=groups, 
                              color='red', 
                              s=10, 
            ...
            # pointplot
            if (group == learning_activity_field) or (group == group_field):
                g = sns.pointplot(data=interactions, 
                                  x=score, 
                                  y=group, 
                                  order=groups, 
            ...
            g = sns.barplot(data=range_counts_frequency_df, 
                            x='count', 
                            y='range')
    ...
    return eval_score_ranges_dict

def add_unique_identifier_code_to_learning_activity_in_sequence(interactions: pd.DataFrame,
                                                                group_field: str,
                                                                user_field: str,
                                                                learning_activity_field: str,
                                                                timestamp_field: str,
    ...
    -------
    tuple
        A tuple containing the interactions dataframe with added indenfier column and the name of the identifier column
    """
...
def generate_evaluation_learning_activity_merge_dict(group_field: str,
                                                     user_field: str,
                                                     learning_activity_field: str,
                                                     learning_activity_code_field: str):
    """Generates a dictionary which maps all evaluation metrics at learning activity level and the input arguments (identifier fields which identify the learning activity for which the evaluation metric is calculated) to an empty list. 
    ...
    """
    # evaluation_learning_activity_fields_list contains all evaluation metric fields on learning activity level and is defined in the constants.py module
    evaluation_learning_activity_merge_dict = {eval_metric: [] for eval_metric in EVALUATION_LEARNING_ACTIVITY_FIELD_LIST}
...
def fill_evaluation_learning_activity_merge_dict(evaluation_learning_activity_merge_dict,
                                                 group_field=None,
                                                 user_field=None,
    ...
    evaluation_learning_activity_merge_dict[learning_activity_field].append(learning_activity)
    evaluation_learning_activity_merge_dict[learning_activity_code_field].append(learning_activity_code)
    ...
    evaluation_learning_activity_merge_dict[EVALUATION_LEARNING_ACTIVITY_SCORE_HIGHEST_FIELD_NAME_STR].append(score_highest)
    evaluation_learning_activity_merge_dict[EVALUATION_LEARNING_ACTIVITY_SCORE_HIGHEST_WITHOUT_HINTS_FIELD_NAME_STR].append(score_highest_without_hint) 
    evaluation_learning_activity_merge_dict[EVALUATION_LEARNING_ACTIVITY_SCORE_FIRST_ATTEMPT_FIELD_NAME_STR].append(score_first_attempt)
    evaluation_learning_activity_merge_dict[EVALUATION_LEARNING_ACTIVITY_SCORE_LAST_ATTEMPT_FIELD_NAME_STR].append(score_last_attempt)
    ...
    evaluation_learning_activity_merge_dict[EVALUATION_LEARNING_ACTIVITY_IS_CORRECT_FIRST_ATTEMPT_WITHOUT_HINTS_FIELD_NAME_STR].append(is_correct_first_attempt_without_hint)
    evaluation_learning_activity_merge_dict[EVALUATION_LEARNING_ACTIVITY_IS_CORRECT_LAST_ATTEMPT_FIELD_NAME_STR].append(is_correct_last_attempt)
    evaluation_learning_activity_merge_dict[EVALUATION_LEARNING_ACTIVITY_IS_CORRECT_LAST_ATTEMPT_WITHOUT_HINTS_FIELD_NAME_STR].append(is_correct_last_attempt_without_hint)
...
def add_evaluation_fields_learning_activity_decorator_with_arguments(interactions_df: pd.DataFrame, 
    ...
    def decorator_add_evaluation_fields_learning_activity(eval_metric_func):
        def wrapper_add_evaluation_fields_learnig_activity(*args, **kwargs):
            ...
            # add learning activity identifier code field -> a sequence of uniterrupted learning activities with the same name will receive the same identifier code
            interactions, learning_activity_code_field = add_unique_identifier_code_to_learning_activity_in_sequence(interactions_df,
                                                                                                                     group_field,
                                                                                                                     user_field,
            ...
            evaluation_learning_activity_merge_dict = generate_evaluation_learning_activity_merge_dict(group_field,
                                                                                                       user_field,
                                                                                                       learning_activity_field,
            ...
            grouping_list = [group_field, user_field, learning_activity_field, learning_activity_code_field]
            ...
            evaluation_learning_activity_df = evaluation_learning_activity_df.replace({False: 0, True: 1})
            ...
            merge_field_list = [group_field, user_field, learning_activity_field, learning_activity_code_field]
            merge_field_list = [i for i in merge_field_list if i]
            interactions = interactions.merge(evaluation_learning_activity_df, how='left', on=merge_field_list)
...
def add_evaluation_fields_learning_activity(calculate_eval_metrics_func,
                                            interactions_df: pd.DataFrame, 
                                            group_field: str,
                                            user_field: str,
                                            learning_activity_field: str,
    ...
    -------
    pd.DataFrame
        The interactins dataframe with added evaluation metrics at the learning activity level
    """
    interactions = add_evaluation_fields_learning_activity_decorator_with_arguments(interactions_df,
                                                                                    group_field,
                                                                                    user_field,
                                                                                    learning_activity_field,
                                                                                    timestamp_field,
                                                                                    order_field)(calculate_eval_metrics_func)(**kwargs)
                                                                    
    return interactions

def add_evaluation_fields_learning_activity_avg(interactions: pd.DataFrame,
    ...
    """
    rename_dict = {k:v for k,v in zip(to_be_averaged_fields_list, averaged_fields_list)}

    group_list = [group_field, user_field]
    group_list = [i for i in group_list if i]
    evaluation_metrics_learning_activity = interactions.groupby(group_list)[to_be_averaged_fields_list].apply(averaging_func, axis=0)
    evaluation_metrics_learning_activity = evaluation_metrics_learning_activity.rename(columns=rename_dict)
    evaluation_metrics_learning_activity = evaluation_metrics_learning_activity.reset_index()

    interactions = interactions.merge(evaluation_metrics_learning_activity, how='left', on=group_list)

    return interactions

def add_evaluation_fields_group_avg(interactions: pd.DataFrame,
                                    to_be_averaged_fields_list: list,
                                    averaged_fields_list: list,
                                    group_field: str,
                                    user_field: str,
    ...
    """
    rename_dict = {k:v for k,v in zip(to_be_averaged_fields_list, averaged_fields_list)}
    evaluation_metrics_group = interactions.groupby([user_field, group_field]).head(1).groupby(user_field)[to_be_averaged_fields_list].apply(averaging_func, axis=0)
    ...
    interactions = interactions.merge(evaluation_metrics_group, how='left', on=user_field)

    return interactions

def add_evaluation_correct_fields(interactions: pd.DataFrame,
                                  evaluation_group_score_field: str,
    ...
    """
    evaluation_is_correct_fields_list = [evaluation_group_is_correct_field, evaluation_course_is_correct_field]
    field_threshold_iterator = zip([evaluation_group_score_field, evaluation_course_score_field], 
                                   [evaluation_group_score_correct_threshold, evaluation_course_score_correct_threshold], 
                                   [relational_operator_group, relational_operator_course], 
                                   [EVALUATION_GROUP_IS_CORRECT_FIELD_NAME_STR, EVALUATION_COURSE_IS_CORRECT_FIELD_NAME_STR],
                                   [add_evaluation_group_is_correct_field, add_evaluation_course_is_correct_field])

    for n, (score_field, threshold, rel_op, is_correct_field_name, add_is_correct_field) in enumerate(field_threshold_iterator):
        if add_is_correct_field:
            interactions[is_correct_field_name] = rel_op(interactions[score_field].apply(lambda x: int(x) if pd.notna(x) else x), threshold)
...
def rename_and_add_empty_evaluation_fields_group_and_course(interactions: pd.DataFrame,
                                                            evaluation_group_score_field: str,
                                                            evaluation_group_is_correct_field: str,
                                                            evaluation_course_score_field: str,
                                                            evaluation_course_is_correct_field: str):
    ...
    """
    field_name_arguments_list = [evaluation_group_score_field,
                                 evaluation_group_is_correct_field,
                                 evaluation_course_score_field,
                                 evaluation_course_is_correct_field]
    evaluation_group_and_course_fields = EVALUATION_GROUP_FIELD_LIST + EVALUATION_COURSE_FIELD_LIST
    eval_field_name_mapping_list = list(zip(field_name_arguments_list,  evaluation_group_and_course_fields))
...
def add_evaluation_group_and_course_fields(interactions: pd.DataFrame,
                                           evaluation_group_score_field: str,
                                           evaluation_group_is_correct_field: str,
                                           evaluation_course_score_field: str,
                                           evaluation_course_is_correct_field: str,
    ...
    """
    interactions, evaluation_is_correct_fields_list = add_evaluation_correct_fields(interactions,
    ...
    interactions = rename_and_add_empty_evaluation_fields_group_and_course(interactions,
                                                                           evaluation_group_score_field,
                                                                           evaluation_group_is_correct_field,
                                                                           evaluation_course_score_field,
                                                                           evaluation_course_is_correct_field)
...
def add_evaluation_fields(interactions: pd.DataFrame,
                          ...
                          calculate_eval_metrics_func,
                          **kwargs):
    """Adds evaluation metrics to the interaction dataframe and returns it. 
    ...
    interactions = add_evaluation_fields_learning_activity(calculate_eval_metrics_func,
                                                           interactions,
                                                           group_field,
                                                           user_field,
                                                           learning_activity_field,
    ...
    if group_field:
        interactions = add_evaluation_fields_learning_activity_avg(interactions,
        ...
        interactions[EVALUATION_GROUP_ALL_LEARNING_ACTIVITIES_MEAN_FIELD_LIST] = len(EVALUATION_GROUP_ALL_LEARNING_ACTIVITIES_MEAN_FIELD_LIST) * [None]
    ...
    interactions = add_evaluation_group_and_course_fields(interactions,
                                                          evaluation_group_score_field,
                                                          evaluation_group_is_correct_field,
                                                          evaluation_course_score_field,
                                                          evaluation_course_is_correct_field,
    ...
    if group_field:
        interactions = add_evaluation_fields_group_avg(interactions, 
        ...
        interactions[EVALUATION_COURSE_ALL_GROUPS_MEAN_FIELD_LIST] = len(EVALUATION_COURSE_ALL_GROUPS_MEAN_FIELD_LIST) * [None]
